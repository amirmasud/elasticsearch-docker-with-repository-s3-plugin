...
RUN proto="$(echo $PLUGIN_INSTALL_PROXY | grep :// | sed -e's,^\(.*://\).*,\1,g')" && \
    ...
    host=${host/:$port} && \
    ES_JAVA_OPTS="-Djdk.http.auth.tunneling.disabledSchemes=\"\" -Djdk.http.auth.proxying.disabledSchemes=\"\" -Dhttp.proxyHost=$host -Dhttp.proxyPort=$port -Dhttp.proxyUser=$user -Dhttp.proxyPassword=$pass" \
